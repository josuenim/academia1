...
def Cursos(request, category_slug=None):
    ...
    if user.is_authenticated:
         #.values_list('curso__id', flat=True): Esto extrae los valores del campo 'curso__id' de los objetos CartItem filtrados. 
        cursos_en_carrito = CartItem.objects.filter(cart__user=user).values_list('curso__id', flat=True)
...
def curso_detail(request, category_slug, curso_slug):
    try:    # get() args: el slug de mi categoria a la pertenece el producto se valide igualandolo con category_slug de la funcion
        ...
        if request.user.is_authenticated:
            in_cart=CartItem.objects.filter(cart__user=request.user, curso = single_curso).exists()# flag .exist devuelve true o false
